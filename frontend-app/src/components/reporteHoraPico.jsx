...
export default function ReporteHoraPico({ data = [], mostActive }) {
  const [barDataText, setBarDataText] = useState("Seleccione una barra para ver detalle.");
  ...
  const barSpacing =
    normalizedData.length > 1
      ? (screenWidth * 0.85 - normalizedData.length * barWidth) / (normalizedData.length - 1) -
        marginSpacing
      : 30;
