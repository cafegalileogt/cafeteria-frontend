...
export default function Home() {
	...
	const handleSaveProduct = async (product) => {
		...
		try {
			...
			if (product.id_producto) {
				...
				setMenuData((prev) =>
					prev.map((p) =>
						p.id_producto === product.id_producto ? { ...p, ...payload } : p,
					),
				);
			} else {
	...
	const handleDeleteProduct = async (id_producto) => {
		...
		try {
			...
			setMenuData((prev) =>
				prev.map((p) =>
					p.id_producto === id_producto ? { ...p, estado: false } : p,
				),
			);
		} catch (err) {
	...
	const handleSaveCategory = async () => {
		try {
			...
			const payload = {
				...
				horario:
					selectedCategory.desde && selectedCategory.hasta
						...
						: "",
				imagen_categoria:
					selectedCategory.imagen_categoria || "ruta/imagen_categoria.jpg",
			};
			...
			if (selectedCategory.id_categoria) {
				...
				setCategories((prev) =>
					prev.map((c) =>
						c.id_categoria === selectedCategory.id_categoria
							? { ...c, ...payload }
							: c,
					),
				);
	...
	const handleDeleteCategory = async (id_categoria) => {
		...
		try {
			...
			setCategories((prev) =>
				prev.map((c) =>
					c.id_categoria === id_categoria ? { ...c, estado: false } : c,
				),
			);
		} catch (err) {
	...
	const renderItem = ({ item }) => (
		<View style={Styles.row}>
			...
			<Text style={[Styles.cellText, { width: "40%" }]} numberOfLines={1}>
				{item.descripcion}
			</Text>
			<View style={[Styles.actions, { width: "15%" }]}>
				...
				<TouchableOpacity
					style={Styles.icons}
					onPress={() => handleDeleteProduct(item.id_producto)}
				>
			...
			<View style={[Styles.actions, { width: "30%" }]}>
				<TouchableOpacity
					...
					onPress={() => {
						...
						const [desde, hasta] = item.horario
							? item.horario.split(" - ")
							: ["", ""];
						setSelectedCategory({
			...
			<View style={Styles.page}>
				<View style={Styles.titleBar}>
					<Text style={Styles.title}>
						{activeTab === "menu" ? "Gestionar Menú" : "Gestionar Categorías"}
					</Text>
				...
				>
					...
					<View
						style={{ height: 1, backgroundColor: "#ccc", marginBottom: 15 }}
					/>
						...
						>
							...
							<View style={{ flex: 1.1 }}>
								...
								<TextInput
									...
									onChangeText={(text) =>
										setSelectedCategory({ ...selectedCategory, nombre: text })
									}
								...
								<View style={{ marginTop: 20 }}>
									...
									<Dropdown
										style={[
											...
											{
												width: "80%",
												opacity: selectedCategory?.todoDia ? 0.5 : 1,
											},
										]}
